...
- **Última actualización:** 07/09/2026. **`/resume` está en producción** desde ese día.
- **Fase 8 (07/09/2026):** copy nuevo del bonus del colofón y **currículum en web en `/resume`** (5ª vista MPA), con enlace en el footer y PDF descargable **generado desde la propia página** (`npm run cv:pdf`). La fuente de verdad del CV pasa a ser `resume.*` en los locales; el PDF de `cv/` (fuera del repo) queda como material histórico.
...
| 20 | Reloj CET **retirado** del header (R6) y sus ficheros **borrados** en T6.10 | Petición del usuario. Al no importarlo nadie, Vite nunca lo metió en el bundle: borrarlo no cambió nada en producción |
...
- Si se toca `/resume` (contenido o diseño): `npm run cv:pdf` y commitear los PDF de `public/cv/`.
...
| 07/09/2026 | T8.1 | Colofón: fuera el copy del «track nuevo», que era simpático pero no decía nada del trabajo. Ahora habla de dónde está el oficio: la latencia que no existe, el estado vacío que no da miedo, el copy que decide. ES + EN + mapa de textos de `plan.md`. |
| 07/09/2026 | T8.2–T8.4 | **`/resume`**: 5ª entrada MPA montada sobre `page.js` (hereda nav, footer, LangToggle e i18n). Formato de referencia `olaolu.dev/resume` —raíl lateral + cuerpo— con la paleta de la casa. Todo el contenido en `resume.*` de los locales, reconstruido en `i18n:change`. Añadido **`?lang=`** en `i18n/index.js` (gana al localStorage): sirve para compartir enlaces por idioma y es lo que necesita el generador de PDF. Enlace `Currículum`/`Resume` en el footer (al `.html`: la URL limpia solo la resuelve nginx) + `location = /resume`. `tests/resume.test.js` compara la forma de los dos diccionarios: si a un idioma le falta un bullet, la página se pintaría a medias sin dar error. 41 tests. |
| 07/09/2026 | T8.5 | PDF del CV **impreso desde la propia página** (`scripts/cv-pdf.sh`): build → `vite preview` → Chrome headless `--print-to-pdf` sobre `?lang=es|en`. La maqueta del PDF es el `@media print` de `resume.css`, así que web y PDF no pueden divergir —a cambio, cada cambio obliga a regenerar. Se imprime contra el build y no contra el dev server: en dev el CSS lo inyecta el HMR por JS y Chrome imprime antes. **Dos bugs:** salía en carta (defecto de Chrome → `size: A4`) y dejaba media hoja en blanco porque `break-inside: avoid` en la entrada entera empujaba el proyecto completo a la página siguiente (el `avoid` pasa a la cabecera y a cada bullet). Y **`.gitignore` tenía `cv/` sin barra inicial, así que también ignoraba `public/cv/`**: el PDF generado no se habría subido nunca → `/cv/`. Dos A4 por idioma. |
| 07/09/2026 | T8.7 | Poda del CV pedida por el usuario: el raíl baja de **33 tecnologías en 4 grupos a 8 ítems** en lista única (`TypeScript · Next.js/React · Python · FastAPI/Node · PostgreSQL · Docker/Cloud · LLM & inferencia local · Agentes & RAG`), y **fuera la sección `EXPERIENCIA`** — el CV va por proyectos. Se cae con ella `.resume__stacklabel` y el bucle de grupos. **Cabo suelto:** Right on it (mar 23 – dic 24), el único puesto pagado, ya no figura en el CV; el peso lo sostiene el `PERFIL`. PDF regenerado (siguen 2 A4, 236 kB en vez de 266). 43 tests. |
| 07/09/2026 | T8.8 | La descarga pasa a **una sola página** (210 mm × lo que haga falta; hoy 484 mm en ES y 479 en EN) en vez de un A4 paginado. Los estilos de papel salen de `@media print` a la clase **`is-paper`**, que activan tanto `?pdf=1` como el evento `beforeprint`: una maqueta, dos destinos (descarga de una hoja / `Cmd+P` paginado). **Tres trampas:** (1) `--print-to-pdf` no espera a que la página mida —de ahí `scripts/print-pdf.mjs`, cliente CDP de 140 líneas sin dependencias, con el `WebSocket` global de Node—; (2) `min-height: 100dvh` de `reset.css`/`layout.css`: al imprimir, `dvh` es el alto de la página y el documento se derrama; (3) la buena: pasando el alto por CDP, Chrome hacía el PDF de 484 mm pero **paginaba el contenido cada 297 mm**, porque el `size: A4` del `@page` del CSS mandaba en la fragmentación. Retirado ese `size` (quien imprime elige papel) y el tamaño lo fija el `@page` inyectado, con `preferCSSPageSize`. El generador avisa en rojo si salen 2 páginas. |
| 07/09/2026 | T8.9 | **Fase 8 desplegada.** `npm run deploy` (resume.html, `cv/` y assets nuevos; borrados los hashes viejos) + `nginx.conf` con el `location = /resume`, copiado tras respaldar el vivo y validarlo en una config de usar y tirar. El vivo no había derivado: el único diff era el nuestro. **Hallazgo:** hay un vhost nuevo `cuquita.juanko.com` (04/09, del usuario) que no está en el repo; declara `listen 443 ssl` **sin** `http2`, así que no choca con el nuestro — la trampa del `duplicate listen options` de T6.7 sigue esquivada, pero conviene recordarla si se añade otro. Smoke test: 9 rutas OK, los dos PDF con sha256 idéntico al local y `Pages: 1`, 404 real, redirecciones conservando ruta. **Cabo suelto menor:** el regex de caché de nginx no incluye `pdf`, así que los CV iban sin `Cache-Control` → cerrado en T8.10. |
| 07/09/2026 | T8.10 | `Cache-Control: public, max-age=86400` para los PDF, en un `location ~* \.pdf$` propio. Un día y no el año de los assets con hash porque la URL del CV es estable pero el fichero cambia con cada `npm run cv:pdf`. El bloque repite las tres cabeceras de seguridad: un `location` con `add_header` propio descarta los heredados (trampa de T6.4). Instalado con el mismo ritual —validación aparte, respaldo, `nginx -t` y reload— y verificado: el PDF sale con la caché nueva y las tres cabeceras, y assets (`immutable` a un año) e imágenes (7 días) conservan la suya. |
